...
C = {
    "bg": "#0d1117", "surface": "#161b22", "surface_alt": "#1c2128",
    "border": "#30363d", "border_light": "#21262d",
    "text": "#e6edf3", "text_muted": "#b1bac4", "text_dim": "#6e7681",
    "accent": "#58a6ff", "accent_dim": "#1f6feb",
    "green": "#3fb950", "green_dim": "#238636",
    "yellow": "#d29922", "yellow_dim": "#9e6a03",
    "red": "#f85149", "red_dim": "#da3633",
    "orange": "#db6d28", "purple": "#bc8cff", "purple_dim": "#8957e5",
    "cyan": "#39d2c0",
}

st.set_page_config(page_title="HeatMind — Heat Intelligence System", page_icon="🔥", layout="wide", initial_sidebar_state="expanded")
...
def svg(name): return SVG.get(name, "")
...
def render_hero():
    ...
    st.markdown(f"""<div class="hero-container">{particles}
...
def cycling_processing(placeholder, escalate=False):
    ...
    if escalate: keys.append("escalating")
    keys.append("complete")
    for k in keys:
        with placeholder.container():
            render_processing_status(k)
            info = next((s for s in STAGES if s["key"] == k), None)
            if info: st.caption(f"{info['emoji']} {info['text']}")
        time.sleep(0.3)

def get_sentiment(score):
    if score is None: return "😐", "neutral", C["text_muted"]
    if score > 0.5: return "😊", "very positive", C["green"]
    if score > 0.2: return "🙂", "positive", C["green"]
    if score > 0.05: return "😐", "slightly positive", C["cyan"]
    if score < -0.5: return "😠", "very negative", C["red"]
    if score < -0.2: return "😕", "negative", C["orange"]
    if score < -0.05: return "😐", "slightly negative", C["yellow"]
...
def render_msg_metadata(msg):
    ...
    with cols[0]:
        ...
        if s is not None:
            ...
            st.markdown(f'<div class="sentiment-pill" style="background:{c}15;color:{c};border:1px solid {c}40;">{e} {l} ({s:.2f})</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="sentiment-pill" style="background:{C["border_light"]};color:{C["text_dim"]};border:1px solid {C["border"]};">— sentiment</div>', unsafe_allow_html=True)
    with cols[1]:
        agent = msg.get("agent", "—")
        ac = {"quick": C["green"], "deep": C["purple"], "emergency": C["red"]}.get(agent, C["text_muted"])
        st.markdown(f'<div class="sentiment-pill" style="background:{ac}15;color:{ac};border:1px solid {ac}40;">{svg("layers")} {agent}</div>', unsafe_allow_html=True)
    with cols[2]:
        rt = msg.get("response_time_ms")
        if rt is not None:
            tc = C["green"] if rt < 500 else C["yellow"] if rt < 2000 else C["red"]
            st.markdown(f'<div class="sentiment-pill" style="background:{tc}15;color:{tc};border:1px solid {tc}40;">⏱ {rt:.0f}ms</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="sentiment-pill" style="background:{C["border_light"]};color:{C["text_dim"]};border:1px solid {C["border"]};">— time</div>', unsafe_allow_html=True)
    with cols[3]:
        ...
        if cs is not None:
            ...
            st.markdown(f'<div class="sentiment-pill" style="background:{cc}15;color:{cc};border:1px solid {cc}40;">{svg("brain")} {label} ({pct:.0f}%)</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="sentiment-pill" style="background:{C["border_light"]};color:{C["text_dim"]};border:1px solid {C["border"]};">— complexity</div>', unsafe_allow_html=True)

def display_error(error_type, message, context=""):
    configs = {
        "CONNECTION_ERROR": {"icon": "🔌", "title": "Connection Error", "color": C["red"], "steps": ["Check if backend is running", "Verify network connection", "Check firewall settings", "Try again in moments"]},
        "TIMEOUT_ERROR": {"icon": "⏱", "title": "Timeout Error", "color": C["yellow"], "steps": ["Request took too long", "Server may be under load", "Try a simpler query", "Try again"]},
        "API_ERROR": {"icon": "⚠", "title": "API Error", "color": C["orange"], "steps": ["API returned error", "Check API key config", "Verify endpoint", "Contact support"]},
    }
    cfg = configs.get(error_type, {"icon": "❌", "title": "Error", "color": C["red"], "steps": ["Unexpected error", "Please try again"]})
    st.markdown(f'<div class="error-box" style="border-left:3px solid {cfg["color"]};"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;"><span style="font-size:1.2rem;">{cfg["icon"]}</span><strong style="color:{cfg["color"]};">{cfg["title"]}</strong></div><div style="color:{C["text_muted"]};font-size:0.9rem;">{message}</div></div>', unsafe_allow_html=True)
    with st.expander("Troubleshooting Steps"):
        for i, step in enumerate(cfg["steps"], 1): st.markdown(f"**{i}.** {step}")
        if st.button("Retry", key=f"retry_{error_type}"): st.rerun()

def display_escalation_banner(reason, ticket_id):
    st.markdown(f'<div class="escalation-banner"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">{svg("siren")}<strong style="font-size:1.1rem;">ESCALATED TO HUMAN AGENT</strong></div><div style="margin:4px 0;"><strong>Reason:</strong> {reason}</div><div style="margin:4px 0;"><strong>Ticket ID:</strong> <code>{ticket_id}</code></div><div style="margin:4px 0;"><strong>Status:</strong> Priority handling requested</div><hr style="border-color:rgba(255,255,255,0.2);margin:8px 0;"><div style="font-style:italic;opacity:0.9;">A human support agent will review this conversation and respond as soon as possible.</div></div>', unsafe_allow_html=True)

def render_escalation_panel():
    escalated = [m for m in st.session_state.messages if m.get("escalated")]
    if not escalated: return
    st.markdown(f'<div class="escalation-banner" style="text-align:center;"><strong>{svg("siren")} {len(escalated)} Active Escalation(s)</strong><div style="opacity:0.9;margin-top:4px;">Human agent intervention requested</div></div>', unsafe_allow_html=True)
    for i, msg in enumerate(reversed(escalated)):
        with st.expander(f"🎫 Ticket #{i+1}: {msg.get('escalation_reason', 'Unknown')} — {msg.get('ticket_id', 'N/A')[:8]}..."):
            st.markdown(f"**Ticket ID:** `{msg.get('ticket_id', 'N/A')}`\n**Reason:** {msg.get('escalation_reason', 'Unknown')}\n**Query:** {msg.get('content', '')[:150]}...")
...
def generate_metrics_export():
    ...
    return {
        ...
        "complexity": {"avg": sum(cs)/len(cs) if cs else None, "high": sum(1 for s in cs if s>0.8), "medium": sum(1 for s in cs if 0.3<=s<=0.8), "low": sum(1 for s in cs if s<0.3)},
    }
...
def handle_query(query):
    ...
    params = {"latitude": loc["latitude"], "longitude": loc["longitude"], "date": datetime.now().strftime("%Y-%m-%d"), "zone": loc["zone"]}
    if not FORTYGUARD_API_KEY:
        return {"response": f"**Routed to:** {routing.agent} agent\n**Complexity:** {routing.complexity.value}\n**Urgency:** {routing.urgency.value}\n\nSet `FORTYGUARD_API_KEY` in `.env` to enable live data.", "agent": routing.agent, "complexity": routing.complexity.value, "response_time_ms": 0, "complexity_score": 0.5, "sentiment_score": 0.0}
    start = time.time()
    try:
        ...
        return {"response": result.get("response", "No response."), "agent": routing.agent, "complexity": routing.complexity.value, "response_time_ms": ms, "complexity_score": 0.5, "sentiment_score": 0.1}
    except Exception as e:
        return {"response": f"Error: {e}", "agent": routing.agent, "complexity": routing.complexity.value, "response_time_ms": (time.time()-start)*1000, "complexity_score": 0, "sentiment_score": -0.5}

def parse_location_from_query(query):
    locs = {"dubai": {"latitude": 25.2048, "longitude": 55.2708, "zone": "Dubai"}, "abu dhabi": {"latitude": 24.4539, "longitude": 54.3773, "zone": "Abu Dhabi"}, "sharjah": {"latitude": 25.3463, "longitude": 55.4209, "zone": "Sharjah"}, "phoenix": {"latitude": 33.4484, "longitude": -112.0740, "zone": "Phoenix"}, "doha": {"latitude": 25.2854, "longitude": 51.5310, "zone": "Doha"}, "riyadh": {"latitude": 24.7136, "longitude": 46.6753, "zone": "Riyadh"}}
    for city, loc in locs.items():
        if city in query.lower(): return loc
...
def render_live_api_metrics():
    ...
    st.markdown(f'<div style="background:{ic}10;border:1px solid {ic}40;border-radius:8px;padding:10px;margin-bottom:16px;display:flex;align-items:center;gap:8px;"><span style="color:{ic};">{i}</span><span style="color:{ic};font-weight:600;font-size:0.9rem;">{msg}</span></div>', unsafe_allow_html=True)
    am = [m for m in st.session_state.messages if m.get("role") == "assistant"]
    if am:
        a1, a2 = st.columns(2)
        with a1:
            ...
            st.markdown(f'<div class="metric-card" style="border-left:3px solid {mc};"><div class="metric-value" style="color:{mc};font-size:1.6rem;">{avg:.0f}ms</div><div class="metric-label">Avg Response Time</div></div>', unsafe_allow_html=True)
        with a2:
            ss = [m["sentiment_score"] for m in am if m.get("sentiment_score") is not None]
            avg_s = sum(ss)/len(ss) if ss else 0
            if avg_s > 0.2: emoji, lbl, mc = "😊", "Positive", C["green"]
            elif avg_s < -0.2: emoji, lbl, mc = "😠", "Negative", C["red"]
            else: emoji, lbl, mc = "😐", "Neutral", C["text_muted"]
            st.markdown(f'<div class="metric-card" style="border-left:3px solid {mc};"><div class="metric-value" style="color:{mc};font-size:1.6rem;">{emoji} {avg_s:.2f}</div><div class="metric-label">{lbl} Sentiment</div></div>', unsafe_allow_html=True)
        a3, a4 = st.columns(2)
        with a3:
            cs = [m["complexity_score"] for m in am if m.get("complexity_score") is not None]
            avg_c = sum(cs)/len(cs) if cs else 0
            if avg_c > 0.8: lbl, mc = "Complex", C["red"]
            elif avg_c > 0.3: lbl, mc = "Moderate", C["orange"]
            else: lbl, mc = "Simple", C["green"]
            st.markdown(f'<div class="metric-card" style="border-left:3px solid {mc};"><div class="metric-value" style="color:{mc};font-size:1.6rem;">{avg_c:.0%}</div><div class="metric-label">{lbl} Queries</div></div>', unsafe_allow_html=True)
        with a4:
            ec = len([m for m in st.session_state.messages if m.get("escalated")])
            mc = C["red"] if ec > 0 else C["green"]
            lbl = f"{ec} Active" if ec else "All Clear"
            st.markdown(f'<div class="metric-card" style="border-left:3px solid {mc};"><div class="metric-value" style="color:{mc};font-size:1.6rem;">{ec}</div><div class="metric-label">{lbl}</div></div>', unsafe_allow_html=True)
...
def main():
    init_session()
    with st.sidebar:
        st.markdown(f'<div style="text-align:center;margin-bottom:20px;"><span style="color:{C["red"]};font-size:1.5rem;">🔥</span><div style="font-size:1.5rem;font-weight:800;color:{C["text"]};letter-spacing:-0.5px;">HeatMind</div><div style="font-size:0.95rem;color:{C["text_muted"]};font-weight:600;margin-top:4px;">Heat Intelligence System</div></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="session-id" style="text-align:center;">{svg("database")} Session: {st.session_state.session_id[:8]}</div>', unsafe_allow_html=True)
        st.divider()
        st.markdown(f'<div class="section-header" style="font-size:1rem;">{svg("map")} Location Settings</div>', unsafe_allow_html=True)
        if st.session_state.last_location:
            st.markdown(f'<div class="zone-card"><div class="zone-name">{st.session_state.last_location["zone"]}</div><div class="zone-coords">{st.session_state.last_location["latitude"]:.4f}, {st.session_state.last_location["longitude"]:.4f}</div></div>', unsafe_allow_html=True)
        else:
            st.info("No location set yet")
        st.divider()
        st.markdown(f'<div class="section-header" style="font-size:1rem;">{svg("activity")} Quick Stats</div>', unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown(f'<div class="section-header" style="font-size:1rem;">{svg("zap")} Actions</div>', unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown(f'<div class="section-header" style="font-size:1rem;">{svg("wifi")} Connection</div>', unsafe_allow_html=True)
        api_ok = bool(FORTYGUARD_API_KEY)
        if api_ok:
            st.markdown(f'<div style="display:flex;align-items:center;gap:6px;color:{C["green"]};font-size:0.9rem;font-weight:700;">{svg("check")} FortyGuard API Connected</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div style="display:flex;align-items:center;gap:6px;color:{C["yellow"]};font-size:0.9rem;font-weight:700;">{svg("alert")} Demo Mode — No API Key</div>', unsafe_allow_html=True)
    ...
    with tab_chat:
        ...
        if prompt := st.chat_input("Ask about heat conditions..."):
            ...
            escalate = "escalat" in prompt.lower() or "urgent" in prompt.lower() or "emergency" in prompt.lower() or "hazard" in prompt.lower() or "danger" in prompt.lower()
            if escalate:
                with status_area.container(): render_processing_status("classifying"); st.caption("🔍 Classifying query type...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("routing"); st.caption("🎯 Routing to optimal agent...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("analyzing"); st.caption("🧠 Analyzing heat data...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("processing"); st.caption("⚡ Processing with AI model...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("sentiment"); st.caption("💬 Analyzing response...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("escalating"); st.caption("🚨 Escalating to human agent...")
                time.sleep(0.15)
                with status_area.container(): render_processing_status("complete"); st.caption("✅ Response ready")
                time.sleep(0.1)
                status_area.empty()
    ...
    with tab_dashboard:
        st.markdown(f'<div class="section-header">{svg("monitor")} Heat Intelligence Dashboard</div>', unsafe_allow_html=True)
        mc1, mc2, mc3, mc4 = st.columns(4)
        with mc1: st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:{C["accent"]};">{st.session_state.query_count}</div><div class="metric-label">Total Queries</div></div>', unsafe_allow_html=True)
        with mc2: st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:{C["red"]};">{st.session_state.alert_count}</div><div class="metric-label">Alerts Triggered</div></div>', unsafe_allow_html=True)
        avg = (st.session_state.total_response_time_ms / st.session_state.query_count) if st.session_state.query_count else 0
        with mc3: st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:{C["green"]};">{avg:.0f}ms</div><div class="metric-label">Avg Response</div></div>', unsafe_allow_html=True)
        with mc4: st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:{C["purple"]};">100%</div><div class="metric-label">Uptime</div></div>', unsafe_allow_html=True)
        ca, cb = st.columns(2)
        with ca:
            ...
            for at in ["quick", "deep", "emergency"]:
                ...
                st.markdown(f'<div style="margin:10px 0;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font-size:1rem;font-weight:700;color:{C["text"]};">{at.title()}</span><span style="font-size:0.9rem;font-weight:600;color:{C["text_muted"]};">{count} ({pct:.0f}%)</span></div><div style="height:8px;background:{C["border_light"]};border-radius:4px;overflow:hidden;"><div style="height:100%;width:{pct}%;background:{color};border-radius:4px;transition:width 0.5s ease;"></div></div></div>', unsafe_allow_html=True)
        with cb:
            st.markdown(f'<div class="section-header">{svg("map")} Active Location</div>', unsafe_allow_html=True)
            st.markdown(f'<div class="zone-card"><div class="zone-name">{st.session_state.last_location["zone"]}</div><div class="zone-coords">{st.session_state.last_location["latitude"]}, {st.session_state.last_location["longitude"]}</div></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="section-header" style="margin-top:16px;">{svg("alert")} Thresholds</div>', unsafe_allow_html=True)
            t1, t2 = st.columns(2)
            with t1: st.markdown(f'<div class="zone-card"><div class="zone-name" style="color:{C["yellow"]};font-size:1.3rem;">{HEAT_THRESHOLD_C}°C</div><div class="zone-coords" style="font-size:0.95rem;">Heat Threshold</div></div>', unsafe_allow_html=True)
            with t2: st.markdown(f'<div class="zone-card"><div class="zone-name" style="color:{C["red"]};font-size:1.3rem;">{HEAT_INDEX_THRESHOLD}</div><div class="zone-coords" style="font-size:0.95rem;">Heat Index</div></div>', unsafe_allow_html=True)
        if st.session_state.messages:
            ...
            for msg in st.session_state.messages[-5:]:
                ...
                st.markdown(f'<div class="timeline-item"><div style="display:flex;align-items:center;gap:8px;"><span style="color:{color};">{icon}</span><span style="font-weight:700;font-size:0.95rem;color:{C["text"]};">{role}</span></div><div class="timeline-content">{msg["content"][:120]}{"..." if len(msg["content"]) > 120 else ""}</div></div>', unsafe_allow_html=True)

    with tab_history:
        st.markdown(f'<div class="section-header">{svg("clock")} Session History</div>', unsafe_allow_html=True)
        if not st.session_state.messages: st.info("No messages yet. Start a conversation in the Chat tab.")
        else:
            for i, msg in enumerate(reversed(st.session_state.messages)):
                role = msg["role"]
                icon = svg("chat") if role == "user" else svg("brain")
                color = C["text_muted"] if role == "user" else C["accent"]
                with st.expander(f"{'You' if role == 'user' else 'Agent'}: {msg['content'][:80]}...", expanded=(i == 0)):
                    st.markdown(msg["content"])
                    if role == "assistant": render_msg_metadata(msg)

    with tab_monitor:
        st.markdown(f'<div class="section-header">{svg("activity")} Autonomous Monitor</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="color:{C["text_muted"]};font-size:1rem;margin-bottom:16px;">Checking every <strong style="color:{C["text"]};font-weight:700;">{MONITOR_INTERVAL_MINUTES} minutes</strong></div>', unsafe_allow_html=True)
        zones = [
        ...
        for z in zones:
            ...
            st.markdown(f'<div class="zone-card" style="border-left:3px solid {bc};"><div style="display:flex;justify-content:space-between;align-items:center;"><div><div class="zone-name">{z["name"]}</div><div class="zone-coords">{z["lat"]}, {z["lng"]}</div></div><span style="display:inline-flex;align-items:center;gap:6px;color:{sc};font-weight:700;font-size:0.95rem;">{si} {st_txt}</span></div></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="section-header">{svg("alert")} Alert Feed</div>', unsafe_allow_html=True)
        if st.session_state.alert_count == 0:
            st.markdown(f'<div class="alert-card-clear"><div style="display:flex;align-items:center;gap:8px;font-weight:700;font-size:1.1rem;">{svg("check")} All Clear</div><div style="margin-top:6px;opacity:0.9;font-size:0.9rem;">System is monitoring. All zones within normal parameters.</div></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="alert-card"><div style="display:flex;align-items:center;gap:8px;font-weight:700;font-size:1.1rem;">{svg("alert")} {st.session_state.alert_count} Alert(s) Triggered</div><div style="margin-top:6px;opacity:0.9;font-size:0.9rem;">Emergency conditions detected in monitored zones.</div></div>', unsafe_allow_html=True)
