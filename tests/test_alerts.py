...
from utils.alerts import send_alert, send_console_alert, send_email_alert, send_webhook_alert
...
class TestSendAlert:
    def test_console_alert_called(self, alert_payload):
        with patch("utils.alerts.send_console_alert") as mock_console, patch("utils.alerts.send_webhook_alert"):
    ...
    def test_webhook_called_when_url_set(self, alert_payload):
        with patch("utils.alerts.send_console_alert"), patch("utils.alerts.send_webhook_alert") as mock_webhook:
            with patch("utils.alerts.send_email_alert"):
    ...
    def test_email_called_when_configured(self, alert_payload):
        with patch("utils.alerts.send_console_alert"), patch("utils.alerts.send_webhook_alert"):
    ...
    def test_email_not_called_when_empty(self, alert_payload):
        with patch("utils.alerts.SMTP_USER", ""), patch("utils.alerts.ALERT_EMAIL_TO", ""):
...
class TestWebhookAlert:
    ...
    def test_webhook_no_url(self, alert_payload):
        with patch("utils.alerts.ALERT_WEBHOOK_URL", ""), patch("utils.alerts.requests.post") as mock_post:
            send_webhook_alert(alert_payload)
...
class TestEmailAlert:
    def test_email_sends(self, alert_payload):
        with patch("utils.alerts.SMTP_USER", "test@test.com"), patch("utils.alerts.SMTP_PASS", "test_pass"):
            with patch("utils.alerts.ALERT_EMAIL_TO", "to@test.com"):
                with patch("utils.alerts.smtplib.SMTP") as mock_smtp:
                    server = MagicMock()
    ...
    def test_email_no_config(self, alert_payload):
        with patch("utils.alerts.SMTP_USER", ""), patch("utils.alerts.SMTP_PASS", ""):
